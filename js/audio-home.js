(() => {
  ...
  document.querySelectorAll("[data-music-source]").forEach(button=>button.addEventListener("click",()=>{document.querySelectorAll("[data-music-source]").forEach(b=>b.classList.remove("active"));button.classList.add("active");state.source=button.dataset.musicSource;load(state.source==="jamendo"?"jamendo":"audius");}));
